...
from build_extraction_chains import build_extraction_chains

# from diskcache import Cache
# from webserver.cache_manager import cache_manager
import dotenv

from workflows.utils import get_redis_connection
...
def process_single_pdf(pdf_path: Path, topic: str) -> dict:
    ...
    # Create cache key from PDF path and topic
    cache_key = hashlib.md5(f"{work_id}_{topic}".encode()).hexdigest()
    cached_result = r.get(cache_key)
    # Check cache
    if cached_result:
        logging.info(f"Using cached result for {work_id}")
        try:
            ...
            cached_result = None
            return None
    logging.info(f"Processing {work_id} (PMID: {pmid})")
